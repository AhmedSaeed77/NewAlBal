...
<?php $__env->startSection('subheaderNav'); ?>
    <a href="<?php echo e(route('library.create')); ?>" class="btn btn-fh btn-white btn-hover-primary font-weight-bold px-2 px-lg-5 mr-2">
...
<?php $__env->startSection('content'); ?>
    <div class="row" id="app-1">
        <?php $__currentLoopData = $CountryPaths; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $paths): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="col-12">
            <div class="card card-custom mt-5">
                <div class="card-body">
                    <h2><?php echo e($paths->first()->country ?? '--'); ?></h2>
                    <table class="table table-bordered table-hover table-checkable" id="kt_datatable1" style="margin-top: 13px !important">
                        ...
                        <tbody>
                            <?php $__currentLoopData = $paths; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $path): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    <td><?php echo e($path->id); ?></td>
                                    <td><?php echo e($path->title); ?></td>
                                    <td>
                                        <button class="btn btn-outline-info" @click="getPath('<?php echo e($path->id); ?>')">
                                            <i class="fa fa-eye"></i> Show
                                        </button>
                                    </td>
                                    <td>
                                        <button class="btn btn-outline-warning" onclick="window.location.href='<?php echo e(route('library.edit', $path->id)); ?>'">
                                            <i class="fa fa-edit"></i> Edit
                                        </button>
                                    </td>
                                    <td>
                                        <button class="btn btn-outline-danger" onclick="AVUtil().deleteConfirmation('<?php echo e(route('library.destroy', $path->id)); ?>', function(url){window.location.href = url;})">
            ...
            <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel"><?php echo e(config('library.path.en')); ?>: {{ path_title }}</h5>
        ...
        app = new Vue({
            ...
            methods: {
                getPath: function(path_id){
                    ...
                    $.ajax ({
                        type: 'POST',
                        url: '<?php echo e(route('library.show', '')); ?>/' + path_id,
