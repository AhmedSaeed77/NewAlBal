...
class LoginController extends Controller
{
    ...
    */
    ...
    public function setupAccount(Request $request)
    {
        ...
        $user = \App\Models\Auth\User::where([
    ...
    public function login(Request $request)
    {
        ...
        $attmpt_user = \App\Models\Auth\User::where('email', $request->input('email'))->get()->first();
        if ($attmpt_user) {
...
//        }
    ...
    protected function sendLoginResponse(Request $request)
    {
        ...
        $loginUser = \App\Models\Auth\User::find($request->session()->get('attempt_user_id'));
        if ($loginUser) {
        ...
        if ($request->withStateResponse) {
            ...
            if ($request->has('targetPage') && $request->has('pageId')) {
                if($request->targetPage == 'package'){
                    if(\App\Models\Package\Packages::find($request->pageId)){
                        return \Redirect::to(route('public.package.view', $request->pageId));
                    }
                }
                if($request->targetPage == 'event'){
                    if(\App\Event::find($request->pageId)){
                        return \Redirect::to(route('public.event.view', $request->pageId));
                    }
                }
