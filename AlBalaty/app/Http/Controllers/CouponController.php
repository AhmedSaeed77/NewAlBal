...
class CouponController extends Controller
{
    ...
    public function promote($coupon_id){
        // demote all
        (new \App\Coupon)->update(['promote' => 0]);
...
//            'qr_code' => 'required|mimes:jpeg,jpg,png,gif',
        ...
        if($req->package_id){
            if(!\App\Models\Package\Packages::find($req->input('package_id')) ){
        ...
        if(!$check){
            ...
            $coupon->total_count = $req->input('coupon_no_use');
            if($req->package_id){
            ...
            if($req->hasFile('qr_code')){
                $coupon->qr_code = $req->file('qr_code')->store('public/payment-QR-Code');
            }
    ...
    public function show(){
        ...
        $coupons = DB::table('coupons')
            ->join('packages', 'coupons.package_id', '=', 'packages.id')
            ->join('admins', 'packages.admin_id', '=', 'admins.id')
            ->select(['coupons.*', 'packages.name AS package_name', 'admins.name AS teacher_name'])
            ->orderBy('created_at', 'desc')
            ->paginate(25);

        return view('super-admin.coupons.show')->with('coupons', $coupons);
    }
