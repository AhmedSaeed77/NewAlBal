...
class API_PackageController extends Controller
{
    ...
    public function search(Request $req){
        ...
        $packages_arr = [];
        $packages = \App\Models\Package\Packages::where('name', 'like', '%'.$req->text.'%')->get();
    ...
    public function packageByCourse(Request $req){
        $packages_arr = [];
        $packages = \App\Models\Package\Packages::where('course_id', $req->course_id)->get();
    ...
    public function VideoComplete(Request $req){
        ...
        $package_ = \App\Models\Package\Packages::find($req->package_id);
        if(!$package_){
    ...
    public function index(Request $req){
        ...
        if($req->popular == 1){
            $packages = \App\Models\Package\Packages::where('active', 1)->where('popular', 1)->orderBy('created_at')->get();
//            return PackageCollection::collection( Packages::where('popular', 1)->get() );
        }else{
            $packages = \App\Models\Package\Packages::where('active', 1)->orderBy('created_at')->get();
//            return PackageCollection::collection( Packages::all() );
    ...
    public function renderPackageDetailsV2($userPackage){
        ...
        if( $userPackage->chapter_included != NULL){
            ...
            foreach($packageChapters as $thisChapter){
                ...
                $item = [
                    ...
                    'questions_number' => $thisChapter->questionsCount,
                    'videos' => $videos_array,
                ];
        ...
        if($userPackage->exams){
            ...
            $packageExams = DB::table('exams')
                ->whereIn('exams.id', $exams_by_id)
                ->join('questions', 'exams.id','like','questions.exam_num')
                ->select(
                    'exams.id',
                    'exams.name',
                    DB::raw('COUNT(*) as questions_number')
    ...
    public function get_top_search(Request $req){
        ...
        $package_selles_list = [];
        $packages = \App\Models\Package\Packages::all();
        if($packages->first()) {
