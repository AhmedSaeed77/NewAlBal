...
class EventController extends Controller
{
   use \App\Payment\Payment;
    ...
    {
         $this->middleware('auth:web,admin,super-admin');
    }
    ...
    public function store(Request $req){
        ...
        $event->description = $req->description ? $req->description : null;
        ...
        $event->path_id = $req->input('path_id');
    ...
    public function myevents(){
        $myp = [];
        $event_user = \App\EventUser::where('user_id', Auth::user()->id)->get();
        foreach($event_user as $es){
            $event = \App\Event::find($es->event_id);
            array_push($myp,$event);
        }
        return view('student.events', compact('event_user','myp'));
    }
