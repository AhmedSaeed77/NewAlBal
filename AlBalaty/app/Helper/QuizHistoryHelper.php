...
trait QuizHistoryHelper
{
    ...
    {
        $dragRightAnswers0 = DB::table('correct_drag_right_answers')->where('quiz_id', $quiz_id);
    ...
    {
        $answers0 = DB::table('correct_answers')->where('quiz_id', $quiz_id);
        $answers = DB::table('wrong_answers')->where('quiz_id', $quiz_id)->union($answers0)->get()->groupBy('question_id');
        return $answers;
    }
    ...
    public function moveDragCenterQuestionFromActive($quiz_id, $user_answers)
    {
        ...
        $wrongAnswers = $answers->filter(function($row)use($user_answers)
        {
            ...
            {
                return $i['question_id'] == $row->question_id && ($i['user_answer_is_correct'] === 'false' || $i['user_answer_is_correct'] === 0);
            });
        ...
        $correctAnswers = $answers->filter(function($row)use($user_answers)
        {
            $thisAnswer = array_filter($user_answers['drag_center'], function($i)use($row)
            {
                return $i['question_id'] == $row->question_id && ($i['user_answer_is_correct'] === 'true' || $i['user_answer_is_correct'] === 1);
            });
            return count($thisAnswer) > 0;
        });
    ...
    {
        $answers = DB::table('active_answers')->where('quiz_id', $quiz_id)->select(
        ...
        /** Insert New Data */
        DB::table('correct_answers')->insert($correctAnswers_arr);
        DB::table('wrong_answers')->insert($wrongAnswers_arr);
    ...
    public function storeMatchingToCenter($quiz_id, $user_answer)
    {
        ...
        DB::table('active_drag_center_answers')->insert($query);
        ...
        foreach($user_answer['right'] as $right)
        {
            if($right)
            {
                array_push($query, [
                    ...
                    'question_drag_left_id'     => $right['left'] ? $right['left']['id']: null,
                    'flaged'            => $user_answer['flag'] == "false" ? 0: 1,
                    'created_at'        => Carbon::now(),
                    'updated_at'        => Carbon::now(),
                ]);
    ...
    {
        $answer = DB::table('active_answers')->updateOrInsert([
                'quiz_id'       => $quiz_id,
                'question_id'   => $user_answer['question_id'],
            ], [
    ...
    public function getInProgressQuizOrCreateNew($thisUser, $package_id, $topic_type, $topic_id, $quiz_data)
    {
