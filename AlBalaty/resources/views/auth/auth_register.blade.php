							...
							<div class="bg-white rounded30 shadow-lg ">
								...
								<div class="px-40 py-20">
									...
									<div class="text-center">
										<p class="mt-15 mb-0">لدى حساب بالفعل ؟<a href="{{ route('login') }}" class="text-danger ml-5"> تسجيل الدخول </a></p>
									</div>
								</div>
							</div>
						</div>
				...
				<div style="display:flex; justify-content: space-between; align-items: center; column-gap:15px;">
									<div class="form-group" style="width:100%;">
										<div class="input-group mb-3">
											...
											<option value="" disabled selected >المراحل التعليميه </option>
												@foreach(\App\Models\Library\Path::all() as $country)
...
<div class="input-group mb-3">
	...
	$("#course"+rowIdx).change(function(){
		...
		$.get('{{route('get.course-parts')}}?country_id='+country_id,function(data){
			$('#lesson'+rowIdx).html(data);
		});
	});
	$('#path'+rowIdx).change(function(){
		let country_id = this.value;
		$.get('{{route('get.path-courses')}}?country_id='+country_id,function(data){
...
<script>
	$(document).ready(function(){
		...
		let country_id = this.value;
		$.get('{{route('get.paths')}}?country_id='+country_id,function(data){
			$('#path').html(data);
		});
		...
		$.get('{{route('get.path-courses')}}?country_id='+path_id,function(data){
			$('#course').html(data);
		});
