                            ...
                            @if(Illuminate\Support\Facades\Input::get('compose') == 'compose')
                                ...
                                <div class="container">
                                    <div class="row" style="border:1px solid #ccc; padding: 10px 15px; border-radius: 10px !important;">
                                        ...
                                        @foreach(\App\Message::where('from_user_id',\App\Models\Auth\User::where('email',Illuminate\Support\Facades\Input::get('toEmail'))->get()->first()->id)->where('from_user_type', 'user')->where('to_user_id',Auth::user()->id)->where('to_user_type', 'admin')->orderBy('created_at', 'desc')->get() as $m )
                                        <div class="col-md-2 col-md-offset-1" style="font-weight: bold;">
                                            {{\App\Models\Auth\User::find($m->from_user_id)->name}} <br> <small>{{$m->created_at}} </small>
                                        </div>
                                    ...
                                    <table class="table table-borderless">
                                        ...
                                        <tbody>
                                            @foreach(\App\Message::where('from_user_id', '=', Auth::user()->id)->where('from_user_type', '=', 'admin')->get() as $message)
                                                @if (\App\Models\Auth\User::find($message->to_user_id))
                                                    <tr>
                                                        <td>{{ \App\Models\Auth\User::find($message->to_user_id)->name }}</td>
                                                        <td>{{ \App\Models\Auth\User::find($message->to_user_id)->email }}</td>
                                    ...
                                    <table class="table table-borderless">
                                        ...
                                        <tbody>
                                            @foreach(\App\Message::where('to_user_id', '=', Auth::user()->id)->where('to_user_type', '=', 'admin')->orderBy('created_at','desc')->get() as $message)
                                                @if (\App\Models\Auth\User::find($message->from_user_id))
                                                <tr>
                                                    <td>{{ \App\Models\Auth\User::find($message->from_user_id)->name }}</td>
                                                    <td>{{ $message->message }}</td>
                                                    <td>
                                                        <a href="{{route('admin.inbox')}}?compose=compose&toEmail={{ \App\Models\Auth\User::find($message->from_user_id)->email }}">Relay</a>
                                                    </td>
