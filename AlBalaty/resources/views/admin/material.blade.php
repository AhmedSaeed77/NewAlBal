    ...
    <div class="card card-custom">
        ...
        <form action="{{route('material.add')}}" method="post" id="materialForm" class="form-horizontal form-bordered" enctype="multipart/form-data">
            ...
            <div class="card-body">
                    ...
                    <div class="col-10">
                        <select name="part_id" id="part_id" class="form-control" v-on:change="getChapters" v-model="part_id">
                            @foreach($teacherParts as $part)
                                <option value="{{$part->part_id}}">{{$part->course_title.' - '.$part->part_title}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    <label class="col-2 col-form-label" for="exampleSelectd1">{{config('library.level.en')}}</label>
                    <div class="col-10">
        ...
        <div class="card-body">
            ...
            <div class="form-horizontal form-md-line-input" style="background: white; padding: 30px 15px; margin-top:20px;">
                <table class="table table-bordered table-hover table-checkable" id="kt_datatable" style="margin-top: 13px !important">
                    ...
                    <tbody>
                    @foreach($teacherMaterials as $q)
                        <tr>
                            <td></td>
                            <td>{{$q->title}}</td>
                            <td>
                                <a target="_blank" href="{{asset('storage/material/'.basename($q->file_url))}}" class=" text-center">
